...
BTN_PAY_DEPOSIT = "🔖 دفع رعبون (جزء هلق + الباقي عند الاستلام)"

# ---------------------------------------------------------------------------
# ح-٢) اختيار وسيلة التحويل (شام كاش أو سيريتل كاش) — بيظهر بعد ما الزبون يختار
# "دفع مسبق" أو "دفع رعبون"، منشان نعرضله معلومات حساب وحدة بس (مو الاثنين سوا).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# ط-٢) دفع رعبون — جزء من المبلغ هلق (تحويل + صورة)، والباقي نقداً عند الاستلام.
...
def deposit_instructions(amount_text: str, service_label: str, account_number: str, account_name: str) -> str:
    ...
    (شوف deposit_instructions_split للحالة العادية: نص المبلغ بالضبط رعبون).
    """
    return (
        f"تمام 👌 هلق حوّل جزء من المبلغ الإجمالي ({amount_text}) كرعبون (متل ما تتفقو عليه)، "
...
def deposit_instructions_split(
    ...
    return (
        ...
        f"هلق حوّل مبلغ الرعبون {deposit_amount} عبر {service_label} عالرقم التالي:\n\n"
        f"📱 {account_number}\n"
        f"👤 باسم: {account_name}\n\n"
        "بعد ما تحوّل، ابعتلنا صورة سكرين شوت لإشعار/إيصال التحويل هون بالشات 📸\n"
...
PAYMENT_LABEL_DEPOSIT = "رعبون (جزء هلق + الباقي عند الاستلام)"
...
def _payment_summary_line(
    ...
    if payment_method == "deposit":
        if for_admin:
            return f"💰 دفع الزبون رعبون: {paid_amount} — الباقي عليه: {remaining_amount}."
        return f"💰 دفعت رعبون {paid_amount}، وباقي عليك {remaining_amount} بتدفعه عند الاستلام."
...
# ---------------------------------------------------------------------------
# /setprices — للتاجر بس: تحديث أسعار الأصناف من جوا تلغرام مباشرة
# ---------------------------------------------------------------------------

SET_PRICES_INTRO = (
    "💵 تحديث الأسعار\n\n"
    "هيدي قائمة كل الأصناف عندك حالياً، مع رقم كل صنف وسعره الحالي (رح توصلك بكذا رسالة):"
)

SET_PRICES_INSTRUCTIONS = (
    "✏️ لتحديث سعر أي صنف، ابعتيلي رقمه والسعر الجديد سوا بسطر واحد، متل:\n"
    "12 55000\n\n"
    "وإذا بدك تحدث أكتر من صنف مرة وحدة، ضيفي كل صنف بسطر لحاله بنفس الرسالة:\n"
    "12 55000\n30 0\n45 48000\n\n"
    "(السعر 0 معناه الصنف ما إلو سعر لهلق، وما رح يظهر للزبون بـ /prices لحد ما تحطيلو سعر حقيقي)\n\n"
    "دوسي /setprices بأي وقت لو حبيتي تشوفي القائمة كاملة من جديد."
)
